...
func (h *greaderHandler) handleReadingListStreamHandler(w http.ResponseWriter, r *http.Request, rm requestModifiers) {
	...
	itemRefs, continuation, err := getItemRefsAndContinuation(r.Context(), *builder, rm)
	if err != nil {
		response.JSONServerError(w, r, err)
		return
	}
...
func getItemRefsAndContinuation(ctx context.Context, builder storage.EntryQueryBuilder, rm requestModifiers) ([]itemRef, int, error) {
